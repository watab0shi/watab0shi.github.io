...
void main(void) {
  ...
  float len = 1.0 / length( cursor - st ) * lightScale;

  st = rotate(st, len * tan( time * 0.5 ) * sin( time * 0.5 ));

  float ptn0 = step( mod( st.x, 0.1 ), lightScale * 0.1 );
  float ptn1 = step( mod( st.y, 0.1 ), lightScale * 0.1 );

  vec3  color  = vec3( 0.0, ptn0, ptn1 );

...
float circle_( vec2 pos, float r ) {
  return clamp(step( length( pos ), r ), 0.0, 1.0);
}
...
float nikukyu_(vec2 st, vec2 pos, float r) {
  float r2 = r * 1.8;
  vec2  p0 = pos;
  float l0 = circle_( p0 - st, r);

  float a1 = PI / 5.0 * 1.0;
  vec2  p1 = p0 + vec2( cos( a1 ), sin( a1 ) ) * r2;
  float l1 = circle_( p1 - st, r * 0.4 );

  float a2 = PI / 5.0 * 2.0;
  vec2  p2 = p0 + vec2( cos( a2 ), sin( a2 ) ) * r2;
  float l2 = circle_( p2 - st, r * 0.45 );

  float a3 = PI / 5.0 * 3.0;
  vec2  p3 = p0 + vec2( cos( a3 ), sin( a3 ) ) * r2;
  float l3 = circle_( p3 - st, r * 0.45 );

  float a4 = PI / 5.0 * 4.0;
  vec2  p4 = p0 + vec2( cos( a4 ), sin( a4 ) ) * r2;
  float l4 = circle_( p4 - st, r * 0.4 );
...
void main(void) {
  ...
  for(float i = 0.0; i < 4.0; i += 1.0) {
    ...
    float y = mod( -time + tan( sin(time * 1.0) ) - 0.75 * i, 3.0 ) - 1.5;
    float r = 0.05 + ( pow( sin( y * 2.0 ), 2.0 ) * 0.5 + 0.5 ) * PI * 0.05;
    mask2 += nikukyu_( st, vec2( x, y ), r );
  }

  mask2 = 1.0-clamp(mask2, 0.0, 1.0);

  vec3 c0 = vec3( 1.0, 0.2, 0.5 );
